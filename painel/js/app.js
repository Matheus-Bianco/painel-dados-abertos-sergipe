 ...
 * Base: template estadual UNESCO/RS — modo SE (IDEB MVP)
...
const SE = {
  ...
  ufNome: 'Sergipe',
  orgao: 'SEED',
  orgaoNome: 'Secretaria de Estado da Educação de Sergipe',
  regionalLabel: 'DRE',
  regionalPlural: 'DREs',
...
function regionalPlural() { return SE_MODE ? 'DREs' : 'CREs'; }
...
const S = {
  ...
  redesData: null,   // 4_1_redes.json — visão cross-rede / mantenedora
  geo: null,
...
function getExportGeoLabel() {
  ...
  if (S.creSel) {
    const rl = regionalLabel();
    return S.creLookup?.cre_list?.find(c => c.cod_cre === S.creSel)?.nome_cre || `${rl} ${S.creSel}`;
  }
  return SE_MODE ? 'Sergipe (estado)' : 'Rio Grande do Sul (estado)';
}
...
async function switchRede(rede) {
  if (rede === S.redeSel && (S.redeCache[rede]?.acesso || S.redeCache[rede]?.ideb)) return;
  ...
  try {
    ...
    if (cached.acesso) {
      ...
      // SE MVP: sem acesso — usa anos/lookup do IDEB
      S.data = {
        serie_temporal: cached.ideb.serie_temporal || {},
        lookup_municipios: cached.ideb.lookup_municipios || {},
        por_municipio: {},
      };
    }
    if (S.data?.serie_temporal) {
      const anos = Object.keys(S.data.serie_temporal).sort();
      const selAno = document.getElementById('sel-ano');
      if (selAno && anos.length) {
    ...
    if (sub) sub.textContent = getRedeLabel() + ufSuffix();
    refreshActiveTab();
  } finally {
    hideRedeLoading();
  }
...
function renderAcesso() {
  ...
  // Garantir botões CSV após filtro município/CRE (applyMunFilter recria gráficos)
  injectExportButtons();
}
...
function applyMunFilter(d, anoSel, lookup) {
  ...
  injectExportButtons();
  // Reforço após paint dos gráficos (filtro município destrói/recria canvas)
...
function renderIdeb() {
  ...
  if (!ideb || !Object.keys(ideb.serie_temporal || {}).length) {
    main.innerHTML = `
      <div class="section-sticky">
        ${sectionBanner('img/icons/nav_ideb.png', 'IDEB', getRedeLabel() + ufSuffix())}
        ${redeToggleHTML()}
      </div>
      <div style="text-align:center;padding:60px 20px;color:var(--text-sec);">
  ...
  // ── Geo label ──
  let geoLabel = getRedeLabel() + ufSuffix();
  ...
  if (S.munSel && lookup[S.munSel]) geoLabel = lookup[S.munSel];
    ...
    geoLabel = creObj ? creObj.nome_cre : `${regionalLabel()} ${S.creSel}`;
  }
  ...
  if (elEvo) {
    // SE: apenas anos observados; RS: anos + metas futuras
    const metaYears = (!SE_MODE && isStateLevel) ? ['2025', '2027'].filter(y => !anos.includes(y)) : [];
  ...
  const idebBuildMap = () => {
    ...
    S.map = L.map('ideb-map-leaflet', { zoomControl: true, scrollWheelZoom: true, attributionControl: false })
      .setView(SE_MODE ? [-10.55, -37.45] : [-29.7, -53.5], SE_MODE ? 7.5 : 6.5);
    L.tileLayer('https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}{r}.png', { maxZoom: 14 }).addTo(S.map);
  ...
  const idebBuildCreMap = () => {
    ...
    creLegend.onAdd = function () {
      const div = L.DomUtil.create('div', 'map-legend');
      div.innerHTML = '<h4>IDEB ' + mapEtapa + ' (' + (SE_MODE ? 'DREs' : 'CREs') + ')</h4>' +
...
function renderHome() {
  ...
  const sections = SE_MODE ? [
    { view: 'ideb', icon: 'img/icons/nav_ideb.png', title: 'IDEB', desc: 'Índice de Desenvolvimento da Educação Básica — redes, DREs e municípios de Sergipe' },
  ] : [
    ...
    <div class="home-wrap">
      <header class="home-banner" role="banner">
        <div class="home-banner-inner">
          <img class="home-banner-flag" src="https://drive.google.com/thumbnail?id=1zin49zFHOsTwJNsW3rCXd6nQh1bSrvq8&sz=w200" alt="Logo SEED Sergipe" onerror="this.style.display='none'">
          <h1 class="home-banner-title">
            <span class="home-banner-title-main">Painel de Dados Abertos</span>
            <span class="home-banner-title-sub">Educação — Sergipe / SEED</span>
          </h1>
        </div>
      </header>
      <div class="home-content">
        ...
        <div class="home-divider" style="margin:8px 0 16px">
          ...
          <span class="home-divider-text">Indicadores Disponíveis</span>
          <span class="home-divider-line"></span>
        </div>

        <div class="home-grid" style="max-width:520px">
          ${sections.map((s, i) => `
            <div class="home-card col-span-2" data-nav="${s.view}" style="animation: fadeSlideUp .5s ease ${.2 + i * .06}s both">
        ...
        <div class="home-grid home-grid-docs">
          <a class="home-card" href="${URL_IDEB}" target="_blank" rel="noopener" style="text-decoration:none">
            <div class="home-card-icon"><img src="img/icons/nav_ideb.png" alt=""></div>
            <div class="home-card-text">
              <div class="home-card-title">Portal do IDEB (INEP)</div>
              <div class="home-card-desc">Resultados oficiais, planilhas e notas técnicas</div>
            </div>
            <span class="home-card-arrow">↗</span>
          </a>
        ...
        <div class="home-footer" style="margin-top:32px">
          <div class="home-footer-text">
            Secretaria de Estado da Educação de Sergipe (SEED)<br>
            Gestão para Aprendizagem · FGV DGPE
          </div>
        </div>
...
const DADOS_TEMAS = [
  ...
  { id: 'infra', icon: 'img/icons/nav_infra.png', nome: 'Infraestrutura',
    ...
    fonte: 'INEP — Censo Escolar' },
  { id: 'icg', icon: 'img/icons/escola.png', nome: 'Complexidade de Gestão',
    arquivo: 'dados_complexidade_gestao.xlsx',
    desc: 'Indicador de Complexidade de Gestão das escolas (ICG), por nível e por município.',
    fonte: 'INEP — Complexidade de Gestão (ICG)' },
  { id: 'inse', icon: 'img/icons/social.png', nome: 'Contexto Socioeconômico (INSE)',
    arquivo: 'dados_inse.xlsx',
    desc: 'Nível socioeconômico médio dos estudantes (INSE), por rede e por município.',
    fonte: 'INEP — Nível Socioeconômico (INSE)' },
  ...
  { id: 'fluxo', icon: 'img/icons/nav_fluxo.png', nome: 'Fluxo e Rendimento',
    arquivo: 'dados_fluxo_rendimento.xlsx',
    desc: 'Taxas de aprovação, reprovação e abandono, por etapa e por série (2010–2025).',
    fonte: 'INEP — Indicadores Educacionais' },
  { id: 'tdi', icon: 'img/icons/politicas.png', nome: 'Distorção Idade-Série',
    arquivo: 'dados_tdi.xlsx',
    desc: 'Taxa de distorção idade-série (TDI) por etapa de ensino e por município.',
    fonte: 'INEP — Distorção Idade-Série' },
  { id: 'saers', icon: 'img/icons/sec_saeb.png', nome: 'SAERS',
    arquivo: 'dados_saers.xlsx',
    desc: 'Proficiências médias e padrões de desempenho do SAERS/RS por etapa e disciplina.',
    fonte: 'SEDUC-RS / CAED — SAERS' },
  { id: 'desigualdades', icon: 'img/icons/nav_desigualdades.png', nome: 'Desigualdades',
    arquivo: 'dados_desigualdades.xlsx',
    desc: 'Resultados do SAERS por recortes de equidade: sexo, raça/cor, deficiência, localização e turno.',
    fonte: 'SEDUC-RS / CAED — SAERS' },
  { id: 'saeb', icon: 'img/icons/sec_saeb.png', nome: 'SAEB',
    arquivo: 'dados_saeb.xlsx',
    desc: 'Proficiências do SAEB em Língua Portuguesa e Matemática, por rede, etapa e município.',
    fonte: 'INEP — SAEB' },
  { id: 'ideb', icon: 'img/icons/nav_ideb.png', nome: 'IDEB',
    arquivo: 'dados_ideb.xlsx',
    desc: 'IDEB observado e metas, por rede, etapa de ensino e por município.',
    fonte: 'INEP — IDEB' },
];
    ...
    if (host) {
      host.innerHTML = `
        <div style="height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;color:var(--text-sec);gap:6px;padding:20px">
          ...
          <div style="font-size:11.5px;max-width:420px;line-height:1.5">As taxas por série (1º–9º ano e 1ª–4ª série EM) estão disponíveis de <strong>2012 em diante</strong>, conforme os arquivos oficiais por município do INEP. Selecione 2012 ou um ano posterior para visualizar.</div>
        </div>`;
    }
    return;
  }
...
function populateCreDropdown() {
  ...
  const todas = SE_MODE ? 'Todas as DREs' : 'Todas as CREs';
  ...
  main.innerHTML = `
    ...
    <div class="section-content" style="padding:10px 16px 50px">
      ...
      <div class="chart-card" style="overflow-x:auto">
        ...
        <div class="chart-source" style="margin-top:8px">${FONTE_CENSO} · Privada inclui escolas filantrópicas (TP_DEPENDENCIA=4).</div>
      </div>
    </div>
  `;
  ...
  S.escolasCreOverlay = L.geoJSON(S.creGeo, {
    ...
    style: (feature) => {
      ...
      return {
        ...
        weight: isActive ? 2.5 : (creCod ? 0.9 : 2.2),
...
async function init() {
  ...
  try {
    if (SE_MODE) {
      ...
      S.data = {
        serie_temporal: S.ideb.serie_temporal || {},
        lookup_municipios: S.ideb.lookup_municipios || {},
        por_municipio: {},
      };
      S.redeCache.estadual = {
        acesso: null, infra: null, fluxo: null, saeb: null, inse: null,
        icg: null, afd: null, ideb: S.ideb, tdi: null, saers: null,
        saersEscolas: null, docentes: null,
      };
    } else {
